...
export async function POST(request) {
  ...
  if (canUseNetlifyBlobs()) {
    ...
    return NextResponse.json({ url: `/api/site-config/afm-entertainment/asset/${encodeURIComponent(id)}` });
  }

  if (canWriteLocalUploads()) {
    const dir = path.join(process.cwd(), "public", "uploads", "afm-entertainment");
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, id), buf);
    return NextResponse.json({ url: `/uploads/afm-entertainment/${id}` });
  }

  return NextResponse.json(
    {
      error:
        "AFM Entertainment image uploads require Netlify Blobs. In the Netlify dashboard, enable Blobs for this site so BLOB_READ_WRITE_TOKEN is available, or run the site locally.",
    },
    { status: 503 }
  );
}
