...
export async function POST(request) {
  ...
  if (canUseNetlifyBlobs()) {
    ...
    return NextResponse.json({ url: `/api/site-config/hero/asset/${encodeURIComponent(id)}` });
  }
  ...
  if (canWriteLocalUploads()) {
    ...
    return NextResponse.json({ url: `/uploads/hero/${id}` });
  }

  return NextResponse.json(
    {
      error:
        "Hero uploads require Netlify Blobs. In the Netlify dashboard, enable Blobs for this site so BLOB_READ_WRITE_TOKEN is available, or run the site locally.",
    },
    { status: 503 }
  );
}
