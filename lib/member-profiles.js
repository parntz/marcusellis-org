...
import { getMemberDirectoryDedupeKey } from "./decode-html-entities.js";
...
export async function listMemberProfileIdsForDirectoryEntry(memberOrTitle) {
  const targetKey =
    typeof memberOrTitle === "string" ? "" : getMemberDirectoryDedupeKey(memberOrTitle);
  if (!targetKey) {
    return [];
  }
  ...
  return rs.rows
    ...
    .filter((row) => getMemberDirectoryDedupeKey(row) === targetKey)
    .map((row) => row.id);
}
