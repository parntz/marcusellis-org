...
import { getReservedAdminAccount, isReservedAdminPasswordMatch } from "./reserved-admins";
...
const providers = [
  ...
  CredentialsProvider({
    ...
    async authorize(credentials) {
      ...
      if (isReservedAdminPasswordMatch(identifier, password)) {
        await ensureReservedAdminUsers();
        const reservedUser = await findAuthUserByIdentifier(identifier);

        if (reservedUser) {
          return {
            id: String(reservedUser.id),
            name: reservedUser.username,
            email: reservedUser.email,
            role: "admin",
            memberPageId: reservedUser.memberPageId,
            memberPageSlug: reservedUser.memberPageSlug,
          };
        }
