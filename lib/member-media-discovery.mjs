...
const DEFAULT_MAX_RESULTS_PER_MEMBER = 10;
...
function log(message) {
  console.error(`[member-media-discovery ${ts()}] ${message}`);
}
...
export async function runMemberMediaDiscovery(overrides = {}) {
  ...
  try {
    ...
    for (const member of members) {
      ...
      if (!query) continue;
      membersProcessed += 1;
      log(`Searching "${query}" for ${memberDisplayName(member)} (#${member.id})`);

      let videoIds = [];
      try {
        videoIds = await searchVideoIds(youtube, query, config.maxResultsPerMember);
      } catch (error) {
        log(`Search failed for member #${member.id}: ${error instanceof Error ? error.message : error}`);
      ...
      try {
        ...
        log(`Snippet fetch failed for member #${member.id}: ${error instanceof Error ? error.message : error}`);
        cursorAfter = member.id;
        continue;
      }
