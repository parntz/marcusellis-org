...
async function findSitePageHeader(client, route) {
  const rs = await client.execute({
    sql: `
...
export async function getEditablePageHeader(
  ...
  const sitePageHeader = await findSitePageHeader(client, route);
  if (sitePageHeader) return sitePageHeader;
...
export async function updateEditablePageHeader({ route: routeInput, title, description }) {
  ...
  // Always mirror admin title/description into page_header_overrides. Content sync scripts can
  // refresh site_pages / news_event_pages from generated imports, and overrides keep admin edits stable.
  await upsertOverrideHeader(client, {
    route,
    title: nextTitle,
