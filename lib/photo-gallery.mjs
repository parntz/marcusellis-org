...
function extractTaggedMembersFromDiscoveryJson(raw) {
  ...
  try {
    ...
    for (const s of sources) {
      ...
      if (!name) continue;
      const mid = String(s.member_id || "").trim();
      const key = mid ? `id:${mid}` : `name:${name.toLowerCase()}`;
      if (seen.has(key)) continue;
      seen.add(key);
      const instrument = String(s.first_primary_instrument || "").trim();
      list.push({
        name,
        memberId: mid || null,
...
export async function listPhotoGalleryItemsPaged({
  ...
  const lim = Math.min(500, Math.max(1, Number(limit) || 96));
  ...
  return {
    items: (listRs.rows || []).map(mapItemRow),
    totalMatching,
    matchingStats,
    limit: lim,
    offset: off,
