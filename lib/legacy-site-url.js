...
function looksLikeBareExternalUrl(value) {
...
export function normalizeLikelyExternalHref(input) {
  const trimmed = String(input || "").trim();
  if (!trimmed) return "";
  if (trimmed.startsWith("/")) return trimmed;
  if (/^[a-z][a-z0-9+.-]*:/i.test(trimmed)) return trimmed;
  if (trimmed.startsWith("//")) return `https:${trimmed}`;
  if (looksLikeBareExternalUrl(trimmed)) return `https://${trimmed.replace(/^\/+/, "")}`;
  return trimmed;
}
...
export function resolveMemberWebsiteHref(canonicalUrl) {
  ...
  const trimmed = normalizeLikelyExternalHref(canonicalUrl);
  try {
    const u = new URL(trimmed, "https://nashvillemusicians.org");
    if (isLegacyHost(u.hostname)) {
...
export function rewriteLegacyNashvilleSiteInHtml(html) {
  ...
  return html
    ...
    .replace(/(\bsrc=["'])\/file\/([^"'?#]+)([^"']*)/gi, `$1${LOCAL_MIRROR_PREFIX}/file/$2--asset$3`)
    .replace(/(\bhref=["'])\/file\/([^"'?#]+)([^"']*)/gi, `$1${LOCAL_MIRROR_PREFIX}/file/$2--asset$3`)
    ...
    .replace(/(\bsrc=["'])\/sites\/default\/themes\//gi, `$1${LOCAL_MIRROR_PREFIX}/sites/default/themes/`)
    .replace(/(\bhref=["'])\/sites\/default\/themes\//gi, `$1${LOCAL_MIRROR_PREFIX}/sites/default/themes/`)
