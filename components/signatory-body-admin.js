...
export function SitePageBodyAdmin({
  route,
  initialSourceHtml = "",
  ...
  const openEditor = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || "Unable to load page body.");
      }
      setDraft(String(data.bodyHtml || ""));
      setOpen(true);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Unable to load page body.");
    } finally {
      setLoadBusy(false);
    }
  }, [route]);
  ...
  async function handleSave(event) {
    ...
    try {
      const res = await fetch("/api/site-page-body", {
        ...
        body: JSON.stringify({ route, bodyHtml: draft }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  return (
    <>
      ...
      <ModalLightbox open={open} onClose={() => !saveBusy && setOpen(false)} closeLabel="Close page editor">
            ...
            <section className="recording-sidebar-modal__section">
              ...
              <div className="recording-page-editor__richtext">
                <NewsEventsBodyEditor
                  ...
                  labelledBy="site-page-body-editor-label"
                />
              </div>
            </section>
...
export const SignatoryBodyAdmin = SitePageBodyAdmin;
